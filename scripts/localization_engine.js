...
// --tw 參數：使用繁體中文字典 (dicts/)，否則使用預設簡體字典 (dicts/)
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (shortcutTrans) {
                    ...
                    newVal = valNorm.replace(/^Refreshes in (\\d+) days?, (\\d+) hours?$/i, (match, d, h) => {
                        return ${false ? 'd + " 天 " + h + " 小時後更新"' : 'd + " 天 " + h + " 小时后刷新"'};
                    });
                } else if (/^Refreshes in (\\d+) hours?, (\\d+) minutes?$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^Refreshes in (\\d+) hours?, (\\d+) minutes?$/i, (match, h, m) => {
                        return ${false ? 'h + " 小時 " + m + " 分鐘後更新"' : 'h + " 小时 " + m + " 分钟后刷新"'};
                    });
                } else if (/^Refreshes in (\\d+) days?$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^Refreshes in (\\d+) days?$/i, (match, d) => {
                        return ${false ? 'd + " 天後更新"' : 'd + " 天后刷新"'};
                    });
                } else if (/^Refreshes in (\\d+) hours?$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^Refreshes in (\\d+) hours?$/i, (match, h) => {
                        return ${false ? 'h + " 小時後更新"' : 'h + " 小时后刷新"'};
                    });
                } else if (/^Refreshes in (\\d+) minutes?$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^Refreshes in (\\d+) minutes?$/i, (match, m) => {
                        return ${false ? 'm + " 分鐘後更新"' : 'm + " 分钟后刷新"'};
                    });
                } else if (/^Learn more about (.+)$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^Learn more about (.+)$/i, (match, p) => {
                        let translatedPreset = p;
                        if (p.toLowerCase() === 'default') translatedPreset = ${false ? '"預設 (Default)"' : '"默认 (Default)"'};
                        else if (p.toLowerCase() === 'full machine') translatedPreset = ${false ? '"全機存取 (Full Machine)"' : '"全机访问 (Full Machine)"'};
                        else if (p.toLowerCase() === 'turbo mode') translatedPreset = ${false ? '"極速模式 (Turbo Mode)"' : '"极速模式 (Turbo Mode)"'};
                        else if (p.toLowerCase() === 'custom') translatedPreset = ${false ? '"自訂 (Custom)"' : '"自定义 (Custom)"'};
                        return ${false ? '"瞭解更多關於 " + translatedPreset + " 的詳細資訊"' : '"了解更多关于 " + translatedPreset + " 的信息"'};
                    });
                    ...
                    newVal = valNorm.replace(/^(\\d+)(s|m|h|d|w|mo|yr)$/i, (match, num, unit) => {
                        ...
                        let unitStr = "";
                        if (unitLower === "s") unitStr = ${false ? '"秒前"' : '"秒前"'};
                        else if (unitLower === "m") unitStr = ${false ? '"分鐘前"' : '"分钟前"'};
                        else if (unitLower === "h") unitStr = ${false ? '"小時前"' : '"小时前"'};
                        else if (unitLower === "d") unitStr = ${false ? '"天前"' : '"天前"'};
                        else if (unitLower === "w") unitStr = ${false ? '"週前"' : '"周前"'};
                        else if (unitLower === "mo") unitStr = ${false ? '"個月前"' : '"个月前"'};
                        else if (unitLower === "yr") unitStr = ${false ? '"年前"' : '"年前"'};
                        return num + unitStr;
                    });
                } else if (/^(.+?): context deadline exceeded$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^(.+?): context deadline exceeded$/i, (match, prefix) => {
                        return prefix + ${false ? '": 請求超時 (context deadline exceeded)"' : '": 请求超时 (context deadline exceeded)"'};
                    });
                } else if (/^(.+?): i\\/o timeout$/i.test(valNorm)) {
                    newVal = valNorm.replace(/^(.+?): i\\/o timeout$/i, (match, prefix) => {
                        return prefix + ${false ? '": I\\/O 超時 (i\\/o timeout)"' : '": I\\/O 超时 (i\\/o timeout)"'};
                    });
                    ...
                    newVal = valNorm.replace(/^Are you sure you want to delete (the |this )?project (.+?)\\??$/i, (match, article, name) => {
                        return ${false ? '"您確定要刪除專案 " + name + " 嗎？"' : '"您确定要删除项目 " + name + " 吗？"'};
                    });
                } else {
                    // 2. 长句子串滑动替换
...
function detectInstallationDir(manualDir) {
    ...
    if (process.platform === 'win32') {
        ...
        addCandidate("D:\\软件安装地址\\antigravity");
...
const newContent = cleanedContent + "\n" + webFrameJs;
    ...
    if (fs.existsSync(updaterPath)) {
        ...
        const targetOptions = `                title: 'Check for Updates',
                ...
                buttons: ['OK'],`;
        const replacementOptions = false
            ? `                title: '檢查更新',
                message: '暫無可用更新',
                buttons: ['確定'],`
            : `                title: '检查更新',
